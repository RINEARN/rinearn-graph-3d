...
/**
 * The window of "Set Renderer" menu.
 */
public class RendererSettingWindow {
	...
	/** The check box to enable/disable the anti-aliasing feature. */
	...
	private final class ComponentInitializer implements Runnable {
		...
		public void run() {
			...
			layout.setConstraints(antialiasingBox, constraints);
			basePanel.add(antialiasingBox);
	...
	private final class ConfigurationReflector implements Runnable {
		...
		private void setJapaneseTexts() {
			frame.setTitle("描画エンジンの設定");
			antialiasingBox.setText("高品質描画（アンチエイリアス）");
			okButton.setText("OK");
		}
		...
		private void setEnglishTexts() {
			frame.setTitle("Set Renderer");
			antialiasingBox.setText("High-Quality (Anti-aliasing)");
			okButton.setText("OK");
		}
		...
		private void updateValuesByConfiguration() {
			RendererConfiguration rendererConfig = configuration.getRendererConfiguration();
			antialiasingBox.setSelected(rendererConfig.isAntialiasingEnabled());
		}
	}
