...
## Ver.5 にあって Ver.6 にまだないものリスト

しばらく塩漬け期間が開いていたため2026年序盤に整理。追い追い更新していく。

間が空いた時の把握用として、このセクションは完成までここに残しておくべき。

* 編集 > 色の設定:

  これは config コンテナ構造とUIの対応どうするのか問題がある。とりあえずUIはシンプルにして、コンテナとの対応は気にし過ぎるべきではない、という判断に過去に一度達した。塩漬け後も判断変わらず。

* ファイル > 設定の保存/読み込み:

  これは設定ファイルの仕様を決める必要がある。設定ファイルを直接いじるユーザーが非常に多いため、結局は初見での直感重視で ini 拡張を維持すべきか。I/Oは要新規実装。ライブラリ化した方がいいか。

* クイック設定ボックス

  設定ファイルI/Oができた時点でこれもすぐできるはず。

* オプション > 等高線（半実装状態）

  オプション自体はあるが、まだ設定画面等がない。遠近調整もまだ。

* オプション > コメントプロット

  データファイルからコメント列を認識して、分離読み込みして格納する処理が要実装。4列ファイルに対して、4列目が全て文字列なら、4列目のみ抽出処理を行った後に、普通の3列データファイルとしてデコードすればよい。

* オプション > カスタムプロット

  これはやっぱり必要。Java/VCSSL/Vnanoで制御スクリプトを書くのは敷居が高すぎるが、多少のプロットコマンドによる細かい操作をしたい、といった場合に。コマンドパーサを書く必要がある。場合によっては、APIからプロットコマンド文字列を投げられるようにしてもいいので、パーサの構造としてそのあたりも考えておく。

* オプション > 平面化

  これは config コンテナ的にどういう扱いにするかという迷いが過去にあったはず。
  CameraConfig管轄で、フラット時のためのパラメータ群を分けて用意するっていう流れになったんだっけか。

* ツール > アニメーション

  これは骨格の設計に関わる。そろそろ練る必要がある。
  -> 練った。後続のセクション参照。

* ツール > ペイント > 指定領域を着色

  これはレンダラー側にそういう層を入れるか？ それともカラーミキサあたりか？
  ペイントツールを将来的にどう拡張していくかにもよる。

* ツール > （旧）数式プロット

  レガシーの中のレガシーだが、慣れている人も居そうなので、新数式ツールにガワだけ旧式をかぶせておいてもいい。
  というか、むしろ新数式ツールを旧式ツールに寄せてテコ入れするか。旧式の関数電卓風UIもこれはこれでアリなので。
  そうすれば、メニューから旧数式ツールをクリックした時に、新数式ツールを立ち上げればいい。

* プログラム メニュー

  これはまず、VCSSLランタイム側で、公開用に整理されたJava用APIを切る必要がある。RG3D 5.x はまだオープンソースではなかったので、散らかった非公開APIを叩いていて汚い。
  とりあえずVnanoのAPIベースに似た形のAPIをそのままかぶせればよさそう。

  併せて、VCSSL側から RG3D Ver.6 を制御するためのプラグインもそろそろ実装する必要がある。
  これは機能的に XCI1 系で十分だが、VCSSLはまだ XCI1 系のプラグインをサポートしていない。
  なのでこの点もVCSSLランタイム側の対応が必要。

  まぁ、GPCI/DPCIベースの層にXCI1をかぶせるのは、仕様が厳格化する方向なので容易なはず。
  逆は仕様が柔らかくなる方向なので難度高いが。


以上が全て揃えば、もうベータリリースくらいはできるのでは。当面は Ver.5.x を続投して並行リリースなわけだし。


## ツール > アニメーション の等のための骨格設計

これは Plotter 実装からのデータアクセスをどうやるかにも関わる、アニメーションだけに留まらない骨格の設計を左右する。
現状では、点プロットなどの Plotter 実装は、PlottingListener 等のインターフェースの枠外の仕様を経由して、データを設定されている。
これを以下のように変更するのはどうか：

* PlottingListener が引数として受け取っている PlottingEvent を拡張し、そこからデータアクセスを提供する
* 具体的には、PlottingEvent.getDataAccessor() などで RinearnGraph3DPlottingDataAccessor を取得できる。で、それの .getX(i, j, k) などで、dataX[i][j][k] を取れる。
* アニメーションモード時などには、i,j,k の組がデータのどこを指すのかを、間接参照アダプタ的に変換するような DataAccessor 実装が渡される。
* Plotter 側は、i, j, k につてどこからどの範囲までをプロットしてほしいのかを知るために、それも DataAccessor に問い合わせる。が、その仕様は練る必要がある。少なくとも系列フィルタ等との整合性は要考慮。
  * 場合によっては系列フィルタ自身もその仕組みで実現されるべきか。それとも独立な概念であるべきか。
    -> 独立な概念でいいと思う。以下の理由:
       * DataAccessor 側の i,j,k 範囲指定でフィルタすると全 Plotter に効くのに対して、V6新機能の系列フィルタは「Plotter ごとに定義できた方が便利だよね」っていう事で入った機能なので。効くべき範囲が違う。
       * 将来的に、アニメーションされる内容が「複数系列が動いていく」感じだった際に、それをフィルタしたい、みたいな事も普通にあり得るし。

  * 仕様、範囲指定よりも、プロット対象の i の数値列や、iを限定した場合の j の数値列、同じく k の数値列を取得できた方が、より応用範囲が広そう。
    -> しかしそれはメッシュとかで煩雑すぎるかも。あるインデックスの次が、どのインデックスに飛ぶべきか、みたいなのが列として渡されると。

  * 仕様、それなら範囲じゃなくて (i):系列 や (i,j):線 や (i,j,k):点の粒度で visibility を問い合わせる感じにしては？ 範囲限定プロットは、visibility が範囲内のみ true になってればよい。
    i,j,k 全範囲について continuous にアクセスは可能で、しかし visibility が false のやつは描画すっ飛ばせばいいよ、的な。それだとメッシュとかでもそこまで煩雑な扱いにはならない。

* で、上記の仕様によって i, j, k 範囲を変化させたりする事によってアニメーションが実現される。

* いやいや、ここまで書いて気付いたが、アニメーションモードは「 i, j, k 範囲を変化させたりする事」だけで十分実現可能では。.getX(i, j, k) の間接参照とかそもそも要る？ 後々のために使えそうではあるけども。
  * そもそも何考えてて思いついた？ どうだっけか
  -> まぁでも言うて PlottingListener 実装が真っ当にAPI経由でデータを取得できる手段は必要なわけで。
     現状は暫定的に、点プロットとかの純正実装 Plotter だけ特別措置をしているわけで、それを解消するために DataAccessor 的なもの自体は必須。
     そうすると後々考えて .getX(i, j, k) みたいに生配列から1段抽象化を挟むのは妥当でしょ。
     -> 確かに。納得。
* 上記の流れからすると、DataAccessor は確かに必須だが、そもそも event 管轄でよいのか？ もっと広範に使わないのか？
  -> とりあえず event 管轄のやつは PlottingListener 実装でのデータ取得を考慮して組んで、より別目的で使うやつが生じた場合は、上層でよりワイドなやつを定義してもいい。
     -> そうすると getter の名前だけ、getDataAccessor() じゃなく getPlottingDataAccessor() みたいに一段明示化しておいた方がいいかも。後でよりワイドなやつの命名で困らないように。
        * 確かに「プロット対象のデータを参照するためのやつ（必要に応じて間接参照とか噛んでたりする）」というニュアンスが明示化されてむしろいいかも。
        * addPlottingListener() とかの既存命名の対応からしても、単に getDataAccessor() だけだと対称にならないので、getPlottingDataAccessor() はその点でもいいかと。

* DataAccessor は世代が進むと必ず仕様が変わっていって、中には非互換のものも生じてくるはず。そのため、世代管理・指定取得機能は必須かもしれない。VCSSLのプラグインインターフェースのように。その仕様どうするか。
  * とりあえず DataAccessor 自体はクラスではなくインターフェースにし、本体側から実装または指定世代アダプタを渡すようにする
  * DataAccessor インターフェースで int getGeneration() で世代確認可能にする。プラグイン仕様ほどの複雑性ではないので、単なる整数番号でよい。文字列で .1 とか .2 とかやると、このレベルだと使う側が面倒すぎる。
  * 取得は getDataAccessor() で現行世代を、getDataAccessor(int generation) で指定世代を取得可能にするとかか。後者はサポート外に関する例外を吐き得る。
    -> いやでも互換がある拡張か非互換拡張かは区別したいので getMajorGeneration() と getMinorGeneration() くらいの区別は欲しいのでは？
       -> それはバージョン決める方はすっきりするが、実装する側は後か先かしか気にしない気がする。
          -> しかし getDataAccessor(int majorGeneration) で、互換のあるメジャーバージョン指定ができるのは便利かも。上位互換あるマイナーバージョンは気にせず、メジャーバージョンのみ指定してその中の最新を返してくれ、っていう。
             * 確かに。そうする。
             -> いや、major-minor を付けるなら generation は不自然な語なので、version にした方がいいのでは？
             * 確かに。多少冗長でも getInterfaceMajorVersion とかの方がいいか。
    -> いや、やっぱりそもそも minor は分けすぎな気がしてきた。メソッド追加毎に minor version 可算していって、呼び出し側で確認しても、想定より古ければメソッド足りなくて叩けないので、どうせエラーで落ちる。
       それはもう「ソフトのバージョンが古い」という話でいい気がする。ソフトのAPIだし、言語のライブラリ/プラグインじゃないので。
       ここで generation が欲しいのは、ソフトの世代が大きく変わって、データの概念やアクセスの手続きとか、そういった思想レベルの変化があった場合に、それによって使う機能セットが全く異なってしまうため。それを検知したい、という話だし。


## 気付いた事（対応が必用な事）

* 等高線を 5.x から移植してパラメータすり合わせしてて思ったけど、どうも遠近オフセットが効いてない気がする。そういえばまだレンダラー側に実装してなかったっけか。どうだっけ。要確認＆まだならそろそろ実装
